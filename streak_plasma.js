 ...
 * Architecture: Standalone Canvas 2D Particle Synthesis (Optimized)
 * =====================================================
 */
  ...
  class StreakPlasmaEngine {
    ...
    getTheme() {
      if (this.streakCount >= 8) {
        return { primary: '#c084fc', secondary: '#ec4899', glow: 'rgba(192, 132, 252, 0.4)', spawnRate: 3, speed: 1.8 };
      } else if (this.streakCount >= 4) {
        return { primary: '#00f3ff', secondary: '#38bdf8', glow: 'rgba(0, 243, 255, 0.35)', spawnRate: 2, speed: 1.4 };
      } else {
        return { primary: '#f59e0b', secondary: '#ef4444', glow: 'rgba(245, 158, 11, 0.3)', spawnRate: 1, speed: 1.0 };
      }
    }
    ...
    burstParticles(count = 15) {
      ...
      for (let i = 0; i < count; i++) {
        ...
        this.particles.push({
          x: 14, y: 12, vx: Math.cos(angle) * speed, vy: Math.sin(angle) * speed,
          radius: Math.random() * 2 + 1, alpha: 1, color: Math.random() > 0.5 ? theme.primary : theme.secondary
        });
      }
    }
    ...
    spawnAmbientParticle() {
      ...
      for (let i = 0; i < theme.spawnRate; i++) {
        this.particles.push({
          x: 12 + (Math.random() * 8 - 4), y: 18, vx: (Math.random() - 0.5) * 0.8, vy: -(Math.random() * theme.speed + 0.8),
          radius: Math.random() * 2 + 0.8, alpha: 0.9, color: Math.random() > 0.4 ? theme.primary : theme.secondary
        });
      }
    }
    ...
    loop() {
      ...
      // CRITICAL FIX: Throttle rendering when the user switches tabs or scrolls away
      if (document.hidden) {
